...
contract DeployAndSetup is Script {
    ...
    function run() public {
        ...
        msc = new MultiSigContract(hc.getETHSepoliaConfig().ispAddress, hc.getETHSepoliaConfig().signatureSchemaId);
        ...
        lm = new LiquidityManager(hc.getETHSepoliaConfig().poolManager, address(vc));
        console2.log("LiquidityManager deployed at:", address(lm));

        // Step 4: Deploy FactoryTokenContract and set in MultiSigContract
        ftc = new FactoryTokenContract(address(msc), address(lm), hc.getETHSepoliaConfig().USDC, msg.sender);
